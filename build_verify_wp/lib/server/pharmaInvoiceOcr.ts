...
const EXPLICIT_SECTION_HEADER_NAMES = [
    "CCL",
    "GENIX",
    "MARTIN DOW",
    "MARTIN DOW MAKER",
...
function normalizeSectionHeadingText(value: unknown) {
...
function looksLikeSectionHeaderItem(rawItem: AnyRecord, resolvedName: string) {
    const normalizedName = normalizeSectionHeadingText(resolvedName);
    if (!normalizedName) {
        return false;
    }

    const exactSectionName = EXPLICIT_SECTION_HEADER_NAMES.includes(normalizedName);
    const headingShape =
        /^[A-Z][A-Z&()/-]{1,22}$/.test(normalizedName) ||
    ...
    return sectionLike && headingShape && (!hasMeaningfulInvoiceRowSignals(rawItem) || exactSectionName);
}
